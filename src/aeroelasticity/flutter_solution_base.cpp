...
MAST::FlutterSolutionBase::~FlutterSolutionBase() {
    ...
    std::vector<MAST::TimeDomainFlutterRootBase*>::iterator
    it = _roots.begin();
...
MAST::FlutterSolutionBase::sort(const MAST::FlutterSolutionBase& sol) {
    ...
    {
        const MAST::TimeDomainFlutterRootBase& r = sol.get_root(i);
        Real max_val = 0.;
        Complex val = 0.;
...
void
MAST::FlutterSolutionBase::init (const MAST::TimeDomainFlutterSolver& solver,
                                 const Real v_ref,
    ...
    for (unsigned int i=0; i<nvals; i++) {
        ...
        _roots[i] = new MAST::TimeDomainFlutterRootBase;
        _roots[i]->init(v_ref,
                        num(i),
                        den(i),
                        _Bmat,
