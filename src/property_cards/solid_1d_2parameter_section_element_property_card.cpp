...
MAST::Solid1D2ParameterSectionProperty::Area::
Area(const std::function<void(Real&, Real&, Real&)> func,
     const std::function<void(Real&, Real&, Real&, Real&, Real&)> dfunc,
...
void MAST::Solid1D2ParameterSectionProperty::Area::
           ...
           Real& m) const 
{
    ...
    _dfunc(DIM1, DIM2, dDIM1, dDIM2, m);
}


MAST::Solid1D2ParameterSectionProperty::TorsionalConstant::
TorsionalConstant(const std::function<void(Real&, Real&, Real&)> func,
...
void MAST::Solid1D2ParameterSectionProperty::TorsionalConstant::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2;
    _DIM1(p, t, DIM1);
...
MAST::Solid1D2ParameterSectionProperty::PolarInertia::
PolarInertia(const std::function<void(Real&, Real&, Real&)> func,
             const std::function<void(Real&, Real&, Real&, Real&, Real&)> dfunc,
             const std::function<void(Real&, Real&, Real&)> Afunc,
             const std::function<void(Real&, Real&, Real&, Real&, 
                                      Real&)> dAfunc,
            const MAST::FieldFunction<Real>& DIM1,
            const MAST::FieldFunction<Real>& DIM2,
...
void MAST::Solid1D2ParameterSectionProperty::PolarInertia::
operator() (const libMesh::Point& p,const Real t, Real& m) const 
{
    Real DIM1, DIM2, offy, offz, A;
    _DIM1(p, t, DIM1);
    _DIM2(p, t, DIM2);
    _hy_offset(p, t, offy);
    _hz_offset(p, t, offz);
            ...
            const Real t, Real& m) const 
{
    ...
    _Afunc(DIM1, DIM2, A);
    _dAfunc(DIM1, DIM2, dDIM1, dDIM2, dA);
...
MAST::Solid1D2ParameterSectionProperty::AreaYMoment::
AreaYMoment(const std::function<void(Real&, Real&, Real&)> Afunc,
            const std::function<void(Real&, Real&, Real&, Real&, Real&)> dAfunc,
            const MAST::FieldFunction<Real>& DIM1,
            const MAST::FieldFunction<Real>& DIM2,
            const MAST::FieldFunction<Real>& hz_offset):
...
void MAST::Solid1D2ParameterSectionProperty::AreaYMoment::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2, off, A;
    _DIM1(p, t, DIM1);
    _DIM2(p, t, DIM2);
    _hz_offset(p, t, off);
            ...
            Real& m) const 
{
    ...
    _Afunc(DIM1, DIM2, A);               _dAfunc(DIM1, DIM2, dDIM1, dDIM2, dA);
...
MAST::Solid1D2ParameterSectionProperty::AreaZMoment::
AreaZMoment(const std::function<void(Real&, Real&, Real&)> Afunc,
            const std::function<void(Real&, Real&, Real&, Real&, Real&)> dAfunc,
            const MAST::FieldFunction<Real>& DIM1,
            const MAST::FieldFunction<Real>& DIM2,
            const MAST::FieldFunction<Real>& hy_offset):
...
void MAST::Solid1D2ParameterSectionProperty::AreaZMoment::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2, off, A;
    _DIM1(p, t, DIM1);
...
MAST::Solid1D2ParameterSectionProperty::AreaInertiaMatrix::
AreaInertiaMatrix(const std::function<void(Real&, Real&, Real&)> Izfunc,
                  const std::function<void(Real&, Real&, Real&, Real&, 
                                           Real&)> dIzfunc,
                  const std::function<void(Real&, Real&, Real&)> Iyfunc,
                  const std::function<void(Real&, Real&, Real&, Real&, 
                                           Real&)> dIyfunc,
                  const std::function<void(Real&, Real&, Real&)> Afunc,
                  const std::function<void(Real&, Real&, Real&, Real&, 
...
void MAST::Solid1D2ParameterSectionProperty::AreaInertiaMatrix::
operator() (const libMesh::Point& p, const Real t, RealMatrixX& m) const 
{
    ...
    _Afunc(DIM1, DIM2, A);
    _Izfunc(DIM1, DIM2, m(0,0));
    _Iyfunc(DIM1, DIM2, m(1,1));
            ...
            RealMatrixX& m) const 
{
    ...
    _Afunc(DIM1, DIM2, A);
    _dAfunc(DIM1, DIM2,  dDIM1, dDIM2, dA);
    _dIzfunc(DIM1, DIM2, dDIM1, dDIM2, m(0,0));
    _dIyfunc(DIM1, DIM2, dDIM1, dDIM2, m(1,1));
