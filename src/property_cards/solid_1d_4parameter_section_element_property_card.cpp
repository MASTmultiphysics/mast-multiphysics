...
MAST::Solid1D4ParameterSectionProperty::Area::
Area(const std::function<void(Real&, Real&, Real&, Real&, Real&)> func,
     const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                            Real&, Real&, Real&, Real&)> dfunc,
...
void MAST::Solid1D4ParameterSectionProperty::Area::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2, DIM3, DIM4;
    _DIM1(p, t, DIM1);
    _DIM2(p, t, DIM2);
    _DIM3(p, t, DIM3);
    _DIM4(p, t, DIM4);
    _func(DIM1, DIM2, DIM3, DIM4, m);
}
           ...
           Real& m) const 
{
    ...
    _dfunc(DIM1, DIM2, DIM3, DIM4, dDIM1, dDIM2, dDIM3, dDIM4, m);
}


MAST::Solid1D4ParameterSectionProperty::TorsionalConstant::
TorsionalConstant(const std::function<void(Real&, Real&, Real&, Real&, 
                                           Real&)> func,
                  const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                           Real&, Real&, Real&, Real&)> dfunc,
                  const MAST::FieldFunction<Real>& DIM1,
...
void MAST::Solid1D4ParameterSectionProperty::TorsionalConstant::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2, DIM3, DIM4;
    _DIM1(p, t, DIM1);
...
MAST::Solid1D4ParameterSectionProperty::PolarInertia::
PolarInertia(const std::function<void(Real&, Real&, Real&, Real&, Real&)> func,
             const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                      Real&, Real&, Real&, Real&)> dfunc,
             const std::function<void(Real&, Real&, Real&, Real&, Real&)> Afunc,
             const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                      Real&, Real&, Real&, Real&)> dAfunc,
             const MAST::FieldFunction<Real>& DIM1,
             const MAST::FieldFunction<Real>& DIM2,
             const MAST::FieldFunction<Real>& DIM3,
             const MAST::FieldFunction<Real>& DIM4,
...
_Afunc(Afunc),
_dAfunc(dAfunc),
_DIM1(DIM1), _DIM2(DIM2), _DIM3(DIM3), _DIM4(DIM4),
...
void MAST::Solid1D4ParameterSectionProperty::PolarInertia::
operator() (const libMesh::Point& p,const Real t, Real& m) const 
{
    Real DIM1, DIM2, DIM3, DIM4, offy, offz, A;
    _DIM1(p, t, DIM1);
    _DIM2(p, t, DIM2);
    _DIM3(p, t, DIM3);
    _hy_offset(p, t, offy);
            ...
            const Real t, Real& m) const 
{
    ...
    _Afunc(DIM1, DIM2, DIM3, DIM4, A);
    _dAfunc(DIM1, DIM2, DIM3, DIM4, dDIM1, dDIM2, dDIM3, dDIM4, dA);
...
MAST::Solid1D4ParameterSectionProperty::AreaYMoment::
AreaYMoment(const std::function<void(Real&, Real&, Real&, Real&, Real&)> Afunc,
            const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                     Real&, Real&, Real&, Real&)> dAfunc,
            const MAST::FieldFunction<Real>& DIM1,
            const MAST::FieldFunction<Real>& DIM2,
...
void MAST::Solid1D4ParameterSectionProperty::AreaYMoment::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2, DIM3, DIM4, off, A;
    _DIM1(p, t, DIM1);
    _DIM2(p, t, DIM2);
    _DIM3(p, t, DIM3);
    _DIM4(p, t, DIM4);
...
MAST::Solid1D4ParameterSectionProperty::AreaZMoment::
AreaZMoment(const std::function<void(Real&, Real&, Real&, Real&, Real&)> Afunc,
            const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                     Real&, Real&, Real&, Real&)> dAfunc,
            const MAST::FieldFunction<Real>& DIM1,
            const MAST::FieldFunction<Real>& DIM2,
...
void MAST::Solid1D4ParameterSectionProperty::AreaZMoment::
operator() (const libMesh::Point& p, const Real t, Real& m) const 
{
    Real DIM1, DIM2, DIM3, DIM4, off, A;
    _DIM1(p, t, DIM1);
...
MAST::Solid1D4ParameterSectionProperty::AreaInertiaMatrix::
AreaInertiaMatrix(const std::function<void(Real&, Real&, Real&, Real&, 
                                           Real&)> Izfunc,
                  const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                           Real&, Real&, Real&, Real&)> dIzfunc,
                  const std::function<void(Real&, Real&, Real&, Real&, 
                                           Real&)> Iyfunc,
                  const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                           Real&, Real&, Real&, Real&)> dIyfunc,
                  const std::function<void(Real&, Real&, Real&, Real&, 
                                           Real&)> Afunc,
                  const std::function<void(Real&, Real&, Real&, Real&, Real&, 
                                           Real&, Real&, Real&, Real&)> dAfunc,
...
void MAST::Solid1D4ParameterSectionProperty::AreaInertiaMatrix::
operator() (const libMesh::Point& p, const Real t, RealMatrixX& m) const 
{
    ...
    _Afunc(DIM1, DIM2, DIM3, DIM4, A);
    _Izfunc(DIM1, DIM2, DIM3, DIM4, m(0,0));
    _Iyfunc(DIM1, DIM2, DIM3, DIM4, m(1,1));
            ...
            RealMatrixX& m) const 
{
    ...
    _Afunc(DIM1, DIM2, DIM3, DIM4, A);
    _dAfunc(DIM1, DIM2, DIM3, DIM4, dDIM1, dDIM2, dDIM3, dDIM4, dA);
    _dIzfunc(DIM1, DIM2, DIM3, DIM4, dDIM1, dDIM2, dDIM3, dDIM4, m(0,0));
    _dIyfunc(DIM1, DIM2, DIM3, DIM4, dDIM1, dDIM2, dDIM3, dDIM4, m(1,1));
