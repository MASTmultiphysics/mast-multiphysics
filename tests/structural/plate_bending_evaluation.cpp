...
BOOST_AUTO_TEST_CASE   (PlateBendingSensitivity) {
    ...
    const libMesh::NumericVector<Real>& sol_vec = solve();
    ...
    for (unsigned int i=0; i<_params_for_sensitivity.size(); i++ ) {
        ...
        solve();
        const libMesh::NumericVector<Real>& dsol_vec = sensitivity_solve(f);
        ...
        const libMesh::NumericVector<Real>& sol_vec1 = solve();
