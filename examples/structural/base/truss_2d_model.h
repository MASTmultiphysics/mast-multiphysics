...
#ifndef __mast_topology_2d_truss_model__
#define __mast_topology_2d_truss_model__
...
init_analysis_mesh(Opt& opt, libMesh::UnstructuredMesh& mesh) {
    ...
    opt._input("truss_dirichletlength_fraction",
    ...
    e_it   = opt._mesh->elements_begin(),
    e_end  = opt._mesh->elements_end();
    ...
    for ( ; e_it != e_end; e_it++) {
        ...
        if ((*e->node_ptr(0))(1) < 1.e-8 &&
            e->centroid()(0) <= length*dirichletlength_fraction)
            opt._mesh->boundary_info->add_side(e, 0, 6);
        else if ((*e->node_ptr(1))(1) < 1.e-8 &&
                 e->centroid()(0) >= length*(1.-dirichletlength_fraction))
            opt._mesh->boundary_info->add_side(e, 0, 7);
        
        if ((*e->node_ptr(0))(0) < 1.e-8 &&
            (*e->node_ptr(0))(1) < 1.e-8 &&
            e->centroid()(0) <= length*dirichletlength_fraction)
            opt._mesh->boundary_info->add_side(e, 0, 8);
    }
    
    opt._mesh->boundary_info->sideset_name(6) = "left_dirichlet";
    opt._mesh->boundary_info->sideset_name(7) = "right_dirichlet";
}
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::init_analysis_dirichlet_conditions(Opt& opt) {
    ...
    *dirichlet  = new MAST::DirichletBoundaryCondition;   // left support
    ...
    opt._discipline->add_dirichlet_bc(6,  *dirichlet);
    ...
    opt._discipline->add_dirichlet_bc(7,  *dirichlet);
    opt._boundary_conditions.insert(dirichlet);
    
    vars = {0};
    dirichlet  = new MAST::DirichletBoundaryCondition;   // left support
    dirichlet->init(8, vars);
    opt._discipline->add_dirichlet_bc(8,  *dirichlet);
    opt._boundary_conditions.insert(dirichlet);
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::init_indicator_dirichlet_conditions(Opt& opt) {
    
    MAST::DirichletBoundaryCondition
    *dirichlet  = new MAST::DirichletBoundaryCondition;   // right boundary
    dirichlet->init(6, opt._indicator_sys_init->vars());
    opt._indicator_discipline->add_dirichlet_bc(6,  *dirichlet);
    opt._boundary_conditions.insert(dirichlet);
    
    dirichlet   = new MAST::DirichletBoundaryCondition;   // left boundary
    dirichlet->init(7, opt._indicator_sys_init->vars());
    opt._indicator_discipline->add_dirichlet_bc(7,  *dirichlet);
    opt._boundary_conditions.insert(dirichlet);
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::init_structural_loads(Opt& opt) {
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::init_indicator_loads(Opt& opt) {
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::init_level_set_dvs(Opt& opt) {
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::initialize_level_set_solution(Opt& opt) {
...
template <typename Opt>
void
MAST::Examples::Truss2DModel::init_simp_dvs(Opt& opt) {
...
#endif // __mast_topology_2d_truss_model__
