...
void
MAST::PanelMesh3DHalfDomain::init (const Real tc,
                                   bool cos_profile,
    ...
    libmesh_assert_equal_to(divs[1]->n_divs(), 2);
    ...
    // make sure that y>=0.
    libmesh_assert_greater_equal(_y0, 0.);
    libmesh_assert_greater_equal(_y1, 0.);
...
void
MAST::PanelMesh3DHalfDomain::process_mesh( ) {
    ...
    for (; n_it != n_end; n_it++)
    {
        ...
        if ((n(0) >= _x0-1.0e-6) && (n(0) <= _x1+1.0e-6) && // x-coord
            (n(1) >= _y0-1.0e-6) && (n(1) <= _y1+1.0e-6))
        {
            x = n(0);
            y = n(1);
            ...
            if (!_cos_profile)
                ...
                sin(.5*pi-_n_maxima_y * 0.5*pi*(y-_y0)/(_y1-_y0));
            else
                n(2) += 0.5*_tc_ratio * ( 1. -(z-_z0)/(_z1-_z0) ) *
                (1. - cos( 2*_n_maxima_x * pi*(x-_x0)/(_x1-_x0))) *
                (1. - cos(.5*pi-2*_n_maxima_y * 0.5*pi*(y-_y0)/(_y1-_y0)));
        }
    }
}
